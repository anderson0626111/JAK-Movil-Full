...
export function AdminLogin({ language, onAuthenticated }: { language: 'ES' | 'EN'; onAuthenticated: (token: string, user: AdminUser, remember: boolean) => void }) {
  ...
  async function register() {
    try {
      ...
      setForgotMessage(data.mensaje || (isEnglish ? 'Registration pending administrator approval.' : 'Registro pendiente de aprobación por el administrador.'));
  ...
  const submit = mode === 'login' ? login : mode === 'register' ? register : (recoveryRequested ? confirmRecovery : requestRecoveryCode);

  return <View style={styles.accessWrap}><View style={styles.accessPanel}>
    <Text style={styles.title}>{mode === 'recover' ? (isEnglish ? 'Recover access' : 'Recuperar acceso') : (isEnglish ? 'Staff access' : 'Acceso del personal')}</Text>
    <Text style={styles.hint}>{mode === 'login' ? (isEnglish ? 'Enter your ID number and password.' : 'Ingresa tu número de cédula y contraseña.') : mode === 'register' ? (isEnglish ? 'Register your ID number and recovery email. Access requires administrator approval.' : 'Registra tu número de cédula y correo de recuperación. El acceso requiere aprobación del administrador.') : (isEnglish ? 'Request a temporary code and create a new password.' : 'Solicita un código temporal y crea una contraseña nueva.')}</Text>
    ...
    {mode === 'register' && <TextInput style={styles.input} value={correo} onChangeText={setCorreo} placeholder={isEnglish ? 'Recovery email' : 'Correo de recuperación'} autoCapitalize="none" keyboardType="email-address" autoComplete="off" importantForAutofill="no" />}
    ...
    {(mode === 'register' || (mode === 'recover' && recoveryRequested)) && <TextInput style={styles.input} value={confirmarContrasena} onChangeText={setConfirmarContrasena} placeholder={isEnglish ? 'Confirm password' : 'Confirmar contraseña'} secureTextEntry autoComplete="new-password" onSubmitEditing={submit} />}
    {(mode === 'register' || (mode === 'recover' && recoveryRequested)) && <Text style={styles.passwordHint}>{isEnglish ? 'Minimum 8 characters: uppercase, lowercase, number and symbol.' : 'Mínimo 8 caracteres: mayúscula, minúscula, número y símbolo.'}</Text>}
