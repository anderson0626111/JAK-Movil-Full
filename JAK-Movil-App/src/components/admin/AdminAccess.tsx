...
export interface AdminUser { id: number; nombre: string; email: string; rol: 'admin' | 'empleado'; activo?: boolean | number; debe_cambiar_contrasena?: boolean | number; foto_url?: string | null; correo_recuperacion?: string | null; }
...
interface EmployeeProfile { id: number; nombre: string; email: string; rol: string; activo?: boolean | number; foto_url?: string | null; correo_recuperacion?: string | null; created_at: string; }
...
export function AdminLogin({ language, onAuthenticated, onCancel }: { language: 'ES' | 'EN'; onAuthenticated: (token: string, user: AdminUser, remember: boolean) => void; onCancel: () => void }) {
  ...
  async function register() {
    try {
      ...
      if (contrasena.length < 8) { setError(isEnglish ? 'Use at least 8 characters' : 'Utiliza al menos 8 caracteres'); return; }
  ...
  async function requestRecoveryCode() {
    try {
      if (!usuario.trim()) { setError(isEnglish ? 'Enter your user or recovery email' : 'Ingresa tu usuario o correo de recuperación'); return; }
  ...
  async function confirmRecovery() {
    try {
      ...
      setForgotMessage(data.mensaje || (isEnglish ? 'Password updated.' : 'Contraseña actualizada.'));
  ...
  return <View style={styles.accessWrap}><View style={styles.accessPanel}>
    ...
    <Text style={styles.hint}>{mode === 'login' ? (isEnglish ? 'Enter your credentials to continue.' : 'Ingresa tus credenciales para continuar.') : mode === 'register' ? (isEnglish ? 'Register with a valid email. Access requires administrator approval.' : 'Regístrate con un correo válido. El acceso requiere aprobación del administrador.') : (isEnglish ? 'Request a temporary code and create a new password.' : 'Solicita un código temporal y crea una contraseña nueva.')}</Text>
    ...
    <TextInput style={styles.input} value={usuario} onChangeText={setUsuario} placeholder={mode === 'register' ? (isEnglish ? 'Valid email' : 'Correo electrónico válido') : (isEnglish ? 'User or recovery email' : 'Usuario o correo de recuperación')} autoCapitalize="none" keyboardType={mode === 'register' ? 'email-address' : 'default'} autoComplete="username" />
    ...
    {(mode !== 'recover' || recoveryRequested) && <TextInput style={styles.input} value={contrasena} onChangeText={setContrasena} placeholder={mode === 'recover' ? (isEnglish ? 'New password' : 'Nueva contraseña') : (isEnglish ? 'Password' : 'Contraseña')} secureTextEntry autoComplete={mode === 'login' ? 'current-password' : 'new-password'} onSubmitEditing={submit} />}
...
export function AdminPanel({ token, user, language, onUserUpdated, onLogout, onBack }: { token: string; user: AdminUser; language: 'ES' | 'EN'; onUserUpdated: (user: AdminUser) => void; onLogout: () => void; onBack: () => void }) {
  ...
  const messageIsError = /^(no |ingresa|debes|debe |faltan|selecciona|completa|el año|el precio|el kilometraje|las contraseñas)/i.test(message);
  const [section, setSection] = useState<'form' | 'available' | 'sold' | 'clients' | 'profiles' | 'account'>(mustChangePassword ? 'account' : 'form');
  ...
  useEffect(() => {
    if (mustChangePassword) {
      setSection('account');
      setMessage(isEnglish ? 'Create a new password to finish recovering your administrator access.' : 'Crea una nueva contraseña para terminar de recuperar el acceso del administrador.');
    }
  }, [mustChangePassword]);
  ...
  async function saveProfile() {
    try {
      ...
      setMessage('');
      const response = await fetch(`${API_URL}/api/auth/perfil`, { method: 'PUT', headers, body: JSON.stringify({ nombre: profileNombre, correo_recuperacion: profileRecoveryEmail, contrasena: profilePassword }) });
      const data = await response.json();
  ...
  async function downloadInvoice(client: ClientSale) {
    try {
      ...
      link.href = url; link.download = `informacion-cliente-${client.id}.pdf`; link.click();
  ...
  function sanitizeFieldValue(key: keyof VehicleForm, value: string) {
    ...
    if (['marca', 'modelo', 'tipo', 'transmision', 'combustible', 'color_exterior'].includes(key)) {
